...
1.流图1初次运行计算频差偏移为33kHz，在流块中添加频移offset之后运行计算频差又变为了23kHz，调节后又存在800Hz的频率偏差，纠正800Hz后又偏移过多，保持800Hz频差在800Hz偏移处接收，部分时间可以解调为未失真音频，等待一段时间后再次出现小幅度偏移
2.流图2，3调制出的频谱显示均为DSB模式，但使用SDR接收时需要使用AM模式才可以正常解调出音频，DSB模式，SSB模式均无法解调出音频
