...
**校准前请将角度轮询调节为0s不轮询，盒子在校准状态下不会对除了K+角度的打点指令以外的任何指令做出回馈**

点击校准后，右侧日志窗口应该会弹出提示**校准开始：请转到 0°，然后发送 K0 确认**,此时将旋转器转至0°(需要在安装旋转器时，将旋转器0°置于正北方)，点击下方校准点确认中的K0按钮,每一段校准的确认为K+对应角度，比如90°点的确认则点击K90按钮，依次类推，当360°点校准完成后，需要根据自己实际的旋转器的限位角度进行旋转，建议旋转角度略小于限位角度，比如说限位角度为432，则建议旋转至425左右作为记录点，然后将实际角度输入到校准点确认部分的输入框，点击K，则确定限位校准点角度以及相关电压回传记录校准点

**注意！角度数据仅支持整数，不要输入小数！**

若使用的是G5500，在校准俯仰角度时最后的180°点也是如此操作
